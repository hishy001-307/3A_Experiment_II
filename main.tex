...
まず、光源とレンズの距離、およびレンズとスクリーンの距離を変化させながら、
スクリーン上の照明状態を観察した。

はじめに、光源の像がスクリーン上に明瞭に結像する位置を探索した。
このとき、スクリーン上には光源の発光素子の構造がはっきりと観察され、
照明強度は高いが一様性に欠ける状態となった。
この配置をクリティカル照明の条件として記録した。

次に、光源をレンズの前側焦点位置に配置し、
光源とレンズの距離が焦点距離 $f$ に等しくなるように調整した。
このとき、レンズ通過後の光はほぼ平行光となり、
スクリーン上では光源像が形成されず、比較的一様な照明が得られた。
スクリーン位置を前後に移動させ、一様性および明るさの変化を確認した。

さらに、スクリーン中央付近をできるだけ明るく、かつ一様に照明する条件を検討するため、
光源とレンズの位置関係を微調整し、
各配置についてスクリーン上の照明分布を比較した。
...
幾何光学的には、レンズ３に平行光を入射し、カメラをその焦点面に置いた場合、
...
（ii）光源像がぼけて一様になるが暗い状態（散光的な一様照明）とを観察し、
